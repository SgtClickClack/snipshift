...
interface ProfessionalOverviewProps {
  bookings: Booking[];
  jobs: Job[];
  onViewChange?: (view: ProfessionalOverviewView) => void;
  stats?: {
...
type ProfessionalOverviewView = 'calendar' | 'profile' | 'earnings' | 'jobs';
...
export default function ProfessionalOverview({ 
  ...
  const upcomingEarnings = useMemo(() => {
    ...
    return bookings
      ...
      .reduce((total: number, booking) => {
        ...
        const rate = parseFloat(String(job.payRate ?? job.hourlyRate ?? job.pay ?? "0"));
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <motion.div variants={itemVariants}>
          <Card className="bg-[#1e293b] border-white/10 group">
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm font-medium text-muted-foreground">Upcoming Earnings</p>
                  <p className={`text-2xl font-bold text-[#10b981] mt-1 ${sensitiveClass}`}>
                    ${upcomingEarnings.toFixed(0)}
                  </p>
                  <p className="text-xs text-muted-foreground mt-1">pending</p>
                </div>
                <div className="rounded-full bg-[#10b981]/15 p-3">
                  <DollarSign className="h-6 w-6 text-[#10b981] transition-transform duration-200 group-hover:scale-105" />
                </div>
              </div>
            </CardContent>
          </Card>
            ...
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                  ...
                  {nextShift ? (
                    <>
                      ...
                      {nextShift.job.location && (
                        <div className="flex items-center gap-2 mt-1 text-xs text-muted-foreground">
                          <MapPin className="h-3 w-3 transition-transform duration-200 group-hover:scale-105" />
                ...
                <div className="rounded-full bg-blue-100 dark:bg-blue-900/20 p-3 ml-2">
                  <Calendar className="h-6 w-6 text-blue-600 dark:text-blue-400 transition-transform duration-200 group-hover:scale-105" />
                </div>
              </div>
            </CardContent>
          </Card>
            ...
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                ...
                <div className="rounded-full bg-purple-100 dark:bg-purple-900/20 p-3">
                  <Eye className="h-6 w-6 text-purple-600 dark:text-purple-400 transition-transform duration-200 group-hover:scale-105" />
                </div>
              </div>
            </CardContent>
          </Card>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="space-y-6">
          ...
          <motion.div variants={itemVariants}>
            <Card className="bg-[#1e293b] border-white/10">
              <Collapsible open={isUpNextOpen} onOpenChange={setIsUpNextOpen}>
                <CardHeader>
                  <div className="flex items-center justify-between gap-2">
                    <CardTitle>Up Next</CardTitle>
                    <CollapsibleTrigger asChild>
                      <Button variant="ghost" size="sm">
                        {isUpNextOpen ? "Hide details" : "View details"}
                      </Button>
                    </CollapsibleTrigger>
                  </div>
                  <p className="text-xs text-muted-foreground mt-1">
                    {upcomingShifts.length > 0
                      ? `${upcomingShifts.length} scheduled • Next: ${
                          nextShift
                            ? `${nextShift.job.title || "Untitled Shift"} — ${format(nextShift.date, "MMM d, h:mm a")}`
                            : "TBD"
                        }`
                      ...
                      <div className="space-y-4">
                        {(upcomingShifts || []).map((shift, index) => (
                          <div
                            ...
                            className="flex items-start justify-between p-4 border border-white/10 rounded-lg hover:bg-muted/50 transition-colors group"
                          >
                            <div className="flex-1 min-w-0">
                              ...
                              <div className="flex items-center gap-3 mt-2 text-sm text-muted-foreground">
                                <div className="flex items-center gap-1">
                                  <Calendar className="h-3 w-3 transition-transform duration-200 group-hover:scale-105" />
                                  <span>
                                    {isToday(shift.date)
                                      ? "Today"
                                ...
                                {shift.job.location && (
                                  <div className="flex items-center gap-1 truncate">
                                    <MapPin className="h-3 w-3 flex-shrink-0 transition-transform duration-200 group-hover:scale-105" />
                                    <span className="truncate">
              ...
              <Collapsible open={isActionItemsOpen} onOpenChange={setIsActionItemsOpen}>
                <CardHeader>
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <CollapsibleTrigger asChild>
                      <Button variant="ghost" size="sm">
                        {isActionItemsOpen ? "Hide details" : "View details"}
                      </Button>
                    </CollapsibleTrigger>
                  </div>
                  <p className="text-xs text-muted-foreground mt-1">
                    {actionItems.length > 0
                      ? `${actionItems.length} pending • ${actionItems[0]?.title || "Next task"}`
                    ...
                    {(actionItems || []).length > 0 ? (
                      <div className="space-y-3">
                        {(actionItems || []).map((item) => (
                          <div
                            key={item.id}
                            className="flex items-start gap-3 p-4 border border-white/10 rounded-lg hover:bg-muted/50 transition-colors cursor-pointer group"
                            onClick={item.action}
                          >
                            ...
                            <ArrowRight className="h-4 w-4 text-muted-foreground flex-shrink-0 mt-1 transition-transform duration-200 group-hover:scale-105" />
                          </div>
                        ))}
                      </div>
                    ) : (
                      <div className="text-center py-8 text-muted-foreground">
                        <CheckCircle2 className="h-12 w-12 mx-auto mb-2 opacity-50 text-[#10b981]" />
        ...
        <div>
          <motion.div variants={itemVariants}>
            <Card className="bg-[#1e293b] border-white/10">
              <Collapsible open={isRecommendedOpen} onOpenChange={setIsRecommendedOpen}>
                <CardHeader>
                  <div className="flex items-center justify-between gap-2">
                    <CardTitle>Recommended for You</CardTitle>
                    <CollapsibleTrigger asChild>
                      <Button variant="ghost" size="sm">
                        {isRecommendedOpen ? "Hide details" : "View details"}
                      </Button>
                    </CollapsibleTrigger>
                  </div>
                  <p className="text-xs text-muted-foreground mt-1">
                    {recommendedJobs.length > 0
                      ? `${recommendedJobs.length} opportunities • ${recommendedJobs[0]?.title || "Top match"}`
                    ...
                    {(recommendedJobs || []).length > 0 ? (
                      <div className="space-y-4">
                        {(recommendedJobs || []).map((job) => (
                          <div
                            key={job.id}
                            className="p-4 border border-white/10 rounded-lg hover:bg-muted/50 transition-colors group"
                          >
                            <div className="flex items-start justify-between mb-3">
                              ...
                              <div className="text-right ml-2">
                                <div className={`text-lg font-bold text-[#10b981] ${sensitiveClass}`}>
                                  ${job.payRate || 'TBD'}
                                </div>
                            ...
                            >
                              <Briefcase className="h-4 w-4 mr-2 transition-transform duration-200 group-hover:scale-105" />
                              Quick Apply
                            </Button>
                          </div>
                        ))}
