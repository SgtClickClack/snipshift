...
export function NotificationToast() {
  ...
  return (
    <div className="fixed bottom-[calc(5rem+env(safe-area-inset-bottom))] sm:bottom-4 right-4 z-[var(--z-toast)] animate-in slide-in-from-bottom-5 fade-in-0 duration-300">
      <div
        className={cn(
          'flex items-start gap-3 p-4 rounded-lg border shadow-lg w-[min(90vw,28rem)] max-w-[90vw]',
          getStyles(latestUnread.type)
        )}
      >
        <Icon className="w-5 h-5 flex-shrink-0 mt-0.5" />
        <div className="flex-1 min-w-0">
          <p className="text-sm font-medium break-words">{latestUnread.message}</p>
        </div>
        <button
          onClick={() => {
            markAsRead(latestUnread.id);
