 ...
 * 
 * Brand Colors:
 * - Neon: hsl(var(--primary)) / text-primary / bg-primary
 * - Dark BG: #0a0a0a
 * 
...
export default function InvestorPortal() {
  ...
  return (
    <>
      ...
      <div 
        className="min-h-screen text-white scroll-smooth selection:bg-primary selection:text-primary-foreground bg-background font-sans"
      >
        {/* Navigation */}
        <nav className="fixed w-full z-50 px-4 md:px-6 py-4 flex justify-between items-center backdrop-blur-md border-b border-white/5 bg-black/20">
          <Link to="/" className="flex items-center gap-2 hover:opacity-80 transition-opacity">
            <span className="text-2xl font-black tracking-tighter italic">
              HOSPO<span className="text-primary">GO</span>
            </span>
          </Link>
          ...
          <div className="flex md:hidden items-center gap-3">
            <button
              ...
              className="p-2 rounded-lg border border-white/10 hover:border-primary/50 transition-colors"
              aria-label="Toggle navigation menu"
            >
              <Menu size={20} className={mobileMenuOpen ? 'text-primary' : 'text-gray-400'} />
            </button>
          </div>
        ...
        {mobileMenuOpen && (
          <div className="fixed top-[72px] left-0 right-0 z-40 md:hidden px-4 py-3 bg-black/95 backdrop-blur-xl border-b border-primary/30 shadow-[0_8px_32px_hsl(var(--primary)/0.1)]">
            <div className="flex flex-col gap-1">
              {NAV_ITEMS.map(({ id, label }) => (
                ...
                >
                  <span>{label}</span>
                  {activeSection === id && (
                    <span className="w-2 h-2 rounded-full bg-primary shadow-[0_0_8px_hsl(var(--primary)/0.8)]" />
                  )}
                </a>
              ))}
              <Button 
        ...
        {showDashboardButton && (
          <Link
            to={getDashboardPath()}
            className="fixed bottom-6 left-6 z-60 flex items-center gap-2 px-5 py-3 rounded-full bg-black/80 backdrop-blur-md border-2 border-primary/40 text-white text-xs font-bold uppercase tracking-widest hover:bg-black hover:border-primary transition-all duration-300 shadow-[0_0_20px_hsl(var(--primary)/0.3)] hover:shadow-[0_0_30px_hsl(var(--primary)/0.5)] group"
        ...
        <section id="trinity" className="py-32 px-6 max-w-7xl mx-auto scroll-mt-24">
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {[
              ...
              <div 
                key={i}
                className="group relative p-10 rounded-[40px] bg-white/5 border border-white/10 transition-all duration-500 hover:border-primary/50 hover:-translate-y-2"
              >
                <div className="mb-8 w-16 h-16 rounded-2xl flex items-center justify-center bg-white/5 group-hover:bg-primary group-hover:text-black transition-all duration-500">
                  {item.icon}
                </div>
        ...
        <section id="vault" className="py-32 px-6 max-w-7xl mx-auto scroll-mt-24">
          <div className="flex flex-col md:flex-row md:items-end justify-between mb-20 gap-8">
            <div className="max-w-2xl">
              <h2 className="text-5xl md:text-7xl font-black mb-6 uppercase italic leading-none tracking-[-0.05em] flex items-center gap-4 flex-wrap">
                <span>Investor <span className="text-primary">Data Room</span></span>
                <ShieldCheck className="w-10 h-10 md:w-14 md:h-14 text-primary animate-pulse" />
              </h2>
            ...
            <div className="flex items-center gap-3 text-[10px] font-bold uppercase tracking-widest text-gray-500 bg-white/5 px-6 py-3 rounded-full border border-white/10 backdrop-blur-sm">
              <Lock size={14} className="text-primary" /> Security: AES-256-GCM Encrypted
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {documents.map((doc) => (
              <div 
                key={doc.id}
                className="group relative p-12 rounded-[48px] bg-white/5 border border-white/10 transition-all duration-700 group-hover:border-primary hover:border-primary hover:-translate-y-3 cursor-pointer overflow-hidden shadow-2xl"
              ...
              >
                ...
                <div className="mb-10 w-20 h-20 rounded-3xl flex items-center justify-center bg-white/5 group-hover:bg-primary group-hover:text-black transition-all duration-500 shadow-xl">
                  {React.cloneElement(doc.icon as React.ReactElement, { size: 32 })}
                </div>
                <h3 className="text-3xl font-bold mb-3 group-hover:text-primary transition-colors">{doc.title}</h3>
                ...
                <div className="flex items-center gap-3 text-[11px] font-bold uppercase tracking-widest text-primary group-hover:gap-5 transition-all">
          ...
          <div className="max-w-7xl mx-auto grid md:grid-cols-2 gap-24 items-center">
            <div className="text-black">
              <h2 className="text-7xl md:text-9xl font-black mb-10 uppercase italic leading-[0.85] tracking-[-0.05em]">
                The <span className="bg-black text-primary px-5 rounded-[40px]">Seed</span> <br />Round
              </h2>
              ...
              <div className="grid grid-cols-2 gap-12">
                <div>
                  ...
                  <div className="text-6xl font-black mb-2 tracking-[-0.05em] italic text-primary drop-shadow-[0_0_20px_hsl(var(--primary)/0.3)]">10.0%</div>
            ...
            <div className="bg-black rounded-[80px] p-12 md:p-20 shadow-[0_40px_100px_rgba(0,0,0,0.5)] border border-white/5">
              <h3 className="text-4xl font-black mb-12 text-primary uppercase italic">Resource Allocation</h3>
          ...
          <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-8">
            ...
            <div className="flex flex-col md:flex-row items-center gap-4 md:gap-8">
              <button
                ...
                className="text-xs text-gray-500 hover:text-primary transition-colors flex items-center gap-2 uppercase tracking-widest font-medium"
              >
            ...
            <div className="relative w-full max-w-5xl bg-[#0d0d0d] border border-white/10 rounded-[64px] shadow-2xl overflow-hidden max-h-[90vh] flex flex-col">
              ...
              <div className="p-10 bg-black border-t border-white/10 flex flex-col md:flex-row justify-between items-center gap-6">
                <div className="flex items-center gap-3 text-[10px] font-bold uppercase tracking-widest text-gray-600 italic">
                  <ShieldCheck size={16} className="text-primary/40" /> Authorized for Executive Review only
                </div>
        ...
        {showRSVPModal && (
          <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-black/95 backdrop-blur-2xl">
            <div 
              className="relative w-full max-w-lg bg-[#0d0d0d] border-2 border-primary/30 rounded-[48px] shadow-[0_0_60px_hsl(var(--primary)/0.15)] overflow-hidden"
              style={{ animation: 'modalSlideIn 0.4s ease-out' }}
            >
              {/* Glow effect */}
              <div className="absolute -top-20 -right-20 w-40 h-40 bg-primary opacity-10 rounded-full blur-3xl pointer-events-none" />
              <div className="absolute -bottom-20 -left-20 w-40 h-40 bg-primary opacity-10 rounded-full blur-3xl pointer-events-none" />
                ...
                {rsvpSuccess && (
                  <>
                    ...
                    <div className="relative inline-block mb-8">
                      ...
                      <div className="relative w-24 h-24 rounded-full bg-primary flex items-center justify-center shadow-[0_0_40px_hsl(var(--primary)/0.5)]">
                        <CheckCircle2 className="w-12 h-12 text-black" />
                      </div>
                    </div>
                    
                    <h3 className="text-3xl font-black uppercase italic text-white mb-4">
                      You're <span className="text-primary">Confirmed</span>
                    </h3>
                    ...
                    <Button 
                      ...
                      className="rounded-full px-12 py-6 text-sm font-bold uppercase tracking-widest bg-primary text-primary-foreground hover:bg-primary/90 shadow-[0_0_30px_hsl(var(--primary)/0.3)]"
                    >
                      Continue Exploring
                    </Button>
                  </>
              ...
              <div className="p-8">
                ...
                <form onSubmit={handleFeedbackSubmit}>
                  <textarea
                    ...
                    rows={4}
                    className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-sm text-white placeholder:text-gray-500 focus:outline-none focus:border-primary/50 transition-colors resize-none mb-4"
                  />
                  ...
                  <div className="flex gap-3 justify-end">
                    ...
                    <Button
                      ...
                      className="bg-primary text-primary-foreground hover:bg-primary/90 rounded-full px-6 font-bold text-xs uppercase tracking-widest"
                    >
