 ...
 *
 * Usage: cd api && node scripts/create-test-user.cjs
 * Or from root: node api/scripts/create-test-user.cjs
 *
 * Requires Firebase Admin SDK (from api deps) and proper credentials
 */
...
// Load environment variables - check multiple locations (paths relative to api/scripts/)
  ...
  try {
    ...
    if (res.rows.length > 0) {
      ...
      const insertRes = await client.query(
        `INSERT INTO users (
          email, name, role, is_onboarded, created_at, updated_at
    ...
    if (user) console.log(`   Firebase UID: ${user.uid}`);
