...
export async function monitorHealth(): Promise<void> {
  try {
    ...
    console.warn('[Health Monitor] System health issue detected:', {
      status: healthCheck.status,
      timestamp: healthCheck.timestamp,
    ...
    const subject = `🚨 HospoGo Health Alert: System ${healthCheck.status.toUpperCase()}`;
    ...
    const html = `
      ...
      <html>
        ...
        <body>
          <div class="container">
            <div class="header">
              <h1 style="margin: 0;">🚨 HospoGo Health Alert</h1>
              <p style="margin: 10px 0 0 0;">System Status: <span class="status-badge status-${healthCheck.status}">${healthCheck.status}</span></p>
            </div>
            <div class="content">
              ...
              <div class="footer">
                <p>This is an automated alert from the HospoGo health monitoring system.</p>
    ...
    console.log('[Health Monitor] Alert email sent successfully to:', ADMIN_EMAIL);
  } catch (error: any) {
