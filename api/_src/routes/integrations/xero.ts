 ...
 * 
 * INVESTOR BRIEFING FIX: Returns [] instead of 404/500 on any error
 * to prevent React crashes in console during investor demos.
 */
router.get('/sync-history', authenticateUser, asyncHandler(async (req: AuthenticatedRequest, res) => {
  const userId = req.user?.id;
  if (!userId) {
    // Return empty array for unauthenticated requests instead of 401
    // This silences console errors during investor portal demos
    res.status(200).json([]);
    return;
  }
  ...
  try {
    ...
    // Wrapped in try/catch to return [] if table doesn't exist or query fails
    ...
    // INVESTOR BRIEFING FIX: Return empty array on any error
    // Prevents console errors and React crashes during demos
    console.warn('[XERO] sync-history query failed, returning []:', err);
    res.status(200).json([]);
  }
}));
