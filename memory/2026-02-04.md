...
## E2E Test Skip Fixes (Session 13)

### Issue: Business-E2E project tests not being skipped correctly

**Root Cause:** The `test.beforeEach(() => { test.skip() })` pattern doesn't properly skip tests in Playwright. The skip was being overridden by nested `beforeEach` blocks inside `test.describe`.

**Solution:** Changed all complex test files to use `test.describe.skip()` pattern instead:

### Files Fixed:
1. **`tests/e2e/calendar-capacity.spec.ts`**
   - Removed incorrect `test.beforeEach()` skip
   - Changed `test.describe('Calendar Capacity E2E Tests'` to `test.describe.skip('Calendar Capacity E2E Tests'`

2. **`tests/e2e/calendar-comprehensive.spec.ts`**
   - Removed incorrect skip pattern
   - Changed all 3 describe blocks to `test.describe.skip()`:
     - `Business Owner: Capacity & Automation Workflow`
     - `Professional: Staff Invitation & Acceptance`
     - `Dual-Context: Full Roster Lifecycle`

3. **`tests/e2e/financial_privacy.spec.ts`**
   - Fixed imports (were placed after beforeEach incorrectly)
   - Changed all 3 describe blocks to `test.describe.skip()`:
     - `Financial Privacy: Business Owner View`
     - `Financial Privacy: Professional Staff View`
     - `Financial Privacy: Dual Context Comparison`
...
### App Code Changes Reviewed (Already in place):
- `professional-calendar.tsx` - Added mounted checks for async state updates
- `JobBoard.tsx` - Added mounted checks for geolocation callbacks
- `push-notifications.ts` - Added proper `isSupported()` check before initializing Firebase Messaging
- `usePushNotifications.ts` - Made foreground handler async
- `conversation.tsx` - Added mounted checks
- `RecommendedShifts.tsx` - Added mounted checks for geolocation

All changes follow proper React cleanup patterns to prevent "Can't perform state update on unmounted component" warnings.

---

## Error #310 Hook Order Fix (Session 14)

### Issue: React Error #310 ("Rendered fewer hooks than expected")

The error was potentially occurring when clicking vacant (RED) shift buckets to open the `ShiftBucketManagementModal`. This error happens when hooks are called after early returns, causing inconsistent hook counts between renders.

### Root Cause Found

In `src/components/calendar/ShiftBucketManagementModal.tsx`:
- Line 114: `if (!bucket) return null;` (early return)
- Lines 120-138: `const assignedStaff = useMemo(...)` (hook AFTER early return)

This violates React's Rules of Hooks - hooks must be called unconditionally and in the same order every render.

### Fix Applied

**`src/components/calendar/ShiftBucketManagementModal.tsx`:**
- Moved `assignedStaff` useMemo BEFORE the early return
- Added null-safe check inside useMemo: `if (!bucket?.bucket?.events) return [];`
- Added explanatory comment documenting the hook order requirement

### Test Infrastructure Created

**`tests/e2e/calendar-crash-repro.spec.ts`:** New E2E test suite with 3 tests:
1. `clicking vacant bucket should open ShiftBucketManagementModal without React error #310`
2. `rapid bucket navigation should maintain hook order stability`
3. `calendar view switches should maintain hook order`

**Enhanced `tests/fixtures/hospogo-fixtures.ts`:**
- Upgraded `ConsoleErrorCollector` with:
  - Separate `criticalErrors` tracking for React hook violations
  - Test annotations for report visibility
  - Real-time console logging of critical errors
  - Diagnostic hints when Error #310 is detected
  - Page error listener for uncaught exceptions

### Test Results
All 4 crash reproduction tests passed:
- ✅ No React Error #310 detected
- ✅ No hook order violations during bucket navigation
- ✅ Console error manifest shows clean state

### Files Modified
- `src/components/calendar/ShiftBucketManagementModal.tsx` - Hook order fix
- `tests/fixtures/hospogo-fixtures.ts` - Enhanced error collector
- `tests/e2e/calendar-crash-repro.spec.ts` - NEW: Crash reproduction test
- `playwright.config.ts` - Added crash-repro to business-e2e project

### Technical Notes
React Error #310 pattern:
```tsx
// ❌ WRONG - useMemo after early return
const [view, setView] = useState('actions');
if (!bucket) return null;
const assignedStaff = useMemo(() => {...}, [bucket.events]); // Error #310!
...
if (!bucket) return null; // Now safe
```
