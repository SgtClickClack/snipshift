...
async function startServer() {
  try {
    ...
    // Start listening (this keeps Node.js alive)
    server.listen(port, "0.0.0.0", () => {
      const address = server.address();
      const boundPort = typeof address === 'object' && address ? address.port : port;
      const mode = isDevelopment ? 'development' : 'production';
      appLogger.serverStart(boundPort, mode);
      log(`Server is ready! Visit: http://localhost:${boundPort}`);
      log(`Startup mode: ${mode} (${isDevelopment ? 'vite-middleware or static fallback' : 'static'})`);
      log(`Listening on 0.0.0.0:${boundPort}`);
    });
