 ...
 * List Users Script
 * 
 * Run with: npx tsx _legacy_snipshift/list-users.ts
 */
...
dotenv.config({ path: path.join(projectRoot, 'api', '.env') });
...
async function run() {
  ...
  try {
    ...
    console.log('   npx tsx _legacy_snipshift/verify-test-user.ts <email>\n');
