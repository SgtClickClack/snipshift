 ...
 * Or with explicit URL: npx tsx run-migration.ts "postgresql://user:pass@host/db"
 * 
...
║    npx tsx run-migration.ts "postgresql://user:pass@neon.tech/db"          ║
║                                                                            ║
...
`);
  ...
  try {
    ...
    const migrationPath = path.join(__dirname, 'api/_src/db/migrations/0036_atomic_settlement_and_productivity_ready.sql');
